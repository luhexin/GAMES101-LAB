...
int main(int argc, const char** argv)
{
    ...
    std::vector<Eigen::Vector3f> pos{{2, 0, -2}, {0, 2, -2}, {-2, 0, -2}};//三个顶点位置

    std::vector<Eigen::Vector3i> ind{{0, 1, 2}};// 顶点坐标索引，0索引x坐标，1索引y坐标

    auto pos_id = r.load_positions(pos);//存储顶点位置，返回对应的id
    auto ind_id = r.load_indices(ind);//存储顶点坐标索引返回对应的id
